...
def EdgeLinking(mag_low, mag_high):
    ...
    # Checking shape of arrays and setting up empty arrays
    rows,cols = mag_high.shape
    edges = np.zeros((rows, cols), dtype=np.uint8)
    visited = np.zeros((rows, cols), dtype=bool)

    # Setting up depth-first search function for recursion
    def dfs(i, j):
        # If this pixel was already visited, return immediately
        if visited[i, j]:
            return
        ...
        # Mark it as an edge in the final result
        edges[i, j] = 255
        ...
        for di in [-1, 0, 1]:
            for dj in [-1, 0, 1]:
                ...
                if 0 <= ni < rows and 0 <= nj < cols:
                    ...
                    # If neighbor is a weak edge and not visited, recursively link it
